...
author: "Andreas Hoelzl [GitHub](https://github.com/andywoodly)"
output:
  html_document:
    keep_md: yes
    toc: yes
...
On the previous results train a combined model (random forrest `rf`) and verify

```{r}
predictionGBMLGA <- data.frame(predictionGBM, predictionLGA, classe=validation$classe)
modelGMALGA <- train(classe ~ ., method="rf", data=predictionGBMLGA)
predictionGMALGA <- predict(modelGMALGA, predictionGBMLGA)
confusionMatrix(predictionGMALGA, predictionGBMLGA$classe)
```

## Evaluate the model on the testing dataset

```{r}
predictionFinal  <- predict(modelGMALGA, testing)
confusionMatrix(predictionFinal, testing$classe)
predictionFinal$finalModel
```
